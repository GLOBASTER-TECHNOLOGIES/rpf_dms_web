...
const Section = ({ title }: { title: string }) => (
  <div className="flex items-center gap-3">
    <span className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.2em] whitespace-nowrap">{title}</span>
...
const EditOfficerModal: React.FC<EditOfficerModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      // UPDATE API expects id in the request BODY
      ...
      toast.error(err?.response?.data?.message ?? "Failed to update officer.", { id: toastId });
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="bg-slate-900 px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-white">Edit Officer</h2>
              <p className="text-slate-400 text-xs mt-0.5">{officer.name}</p>
            </div>
          </div>
        ...
        <div className="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <Field label="Force Number" icon={<Hash size={15} />}>
            <input name="forceNumber" type="text" className={`${inputCls} font-mono`} value={form.forceNumber} onChange={handleChange} />
          </Field>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Rank" required icon={<ShieldCheck size={15} />}>
              <select name="rank" className={selectCls} value={form.rank} onChange={handleChange}>
            ...
            <Field label="Role" required icon={<ShieldCheck size={15} />}>
              <select name="role" className={selectCls} value={form.role} onChange={handleChange}>
          ...
          <Field label="Division" icon={<Layers size={15} />}>
            <input name="division" type="text" className={`${inputCls} uppercase`} placeholder="E.G. TRICHY" value={form.division} onChange={handleChange} />
          </Field>
          ...
          <div className="flex items-center justify-between bg-slate-50 px-4 py-3 rounded-xl border border-slate-200">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Inactive officers cannot log in.</p>
            </div>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50">
          <button onClick={onClose} disabled={saving} className="flex-1 py-3 rounded-xl border border-slate-200 text-slate-600 text-sm font-semibold hover:bg-slate-100 transition disabled:opacity-50 bg-white">
            Cancel
          </button>
          <button onClick={handleSubmit} disabled={saving} className="flex-1 py-3 rounded-xl bg-slate-900 text-white text-sm font-bold hover:bg-slate-800 transition flex items-center justify-center gap-2 disabled:opacity-60">

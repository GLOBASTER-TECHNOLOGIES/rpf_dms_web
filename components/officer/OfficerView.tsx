...
const DeleteConfirmModal = ({
  officer, onConfirm, onCancel, loading,
}: {
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
      ...
      <div className="px-6 py-5">
        ...
        <p className="text-xs text-slate-400 mt-1.5">This action is permanent and cannot be undone.</p>
      </div>
      <div className="flex gap-3 px-6 py-4 bg-slate-50 border-t border-slate-100">
        <button onClick={onCancel} disabled={loading}
          className="flex-1 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-600 text-sm font-semibold hover:bg-slate-50 transition disabled:opacity-50">
          Cancel
        </button>
        <button onClick={onConfirm} disabled={loading}
          className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-bold flex items-center justify-center gap-2 transition disabled:opacity-60">
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 flex flex-col overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        ...
        <div className="flex items-start gap-3">
          ...
          <span className={`flex-shrink-0 flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${
            officer.active ? "bg-green-50 text-green-700" : "bg-slate-100 text-slate-400"
          }`}>
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider ${role.pill}`}>
            {officer.role}
          </span>
        </div>
      ...
      <div className="flex border-t border-slate-100">
        <button onClick={onEdit}
          className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-semibold text-slate-500 hover:bg-blue-50 hover:text-blue-600 transition border-r border-slate-100">
          <Edit size={13} /> Edit
        </button>
        <button onClick={onDelete}
          className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-semibold text-slate-400 hover:bg-red-50 hover:text-red-500 transition">
  ...
  const filtered = useMemo(() => {
    ...
    return officers.filter((o) => {
      const matchQ = !q
        ...
        || (o.forceNumber ?? "").toLowerCase().includes(q)
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-30">
        <div className="px-6 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-bold text-slate-900 leading-tight">Police Officers</h1>
          ...
          <div className="flex items-center gap-2">
            <button onClick={exportCSV} disabled={filtered.length === 0}
              className="flex items-center gap-1.5 bg-slate-900 hover:bg-black text-white px-3.5 py-2.5 rounded-xl text-xs font-bold transition disabled:opacity-40">
              <FileDown size={14} /> Export
            </button>
            <button onClick={() => setShowCreateForm(true)}
              className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white px-3.5 py-2.5 rounded-xl text-xs font-bold transition shadow-md shadow-blue-100">
      ...
      <div className="px-6 py-5 space-y-4 max-w-screen-xl mx-auto">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl border text-xs font-bold transition shadow-sm ${
              showFilters || activeFilters > 0
                ? "bg-blue-600 text-white border-blue-600 shadow-blue-100 shadow-md"
        ...
        {!loading && officers.length > 0 && (
          <div className="flex items-center justify-between">
            <p className="text-xs text-slate-500">
              Showing <span className="font-bold text-slate-700">{filtered.length}</span>
              {" "}of <span className="font-bold text-slate-700">{officers.length}</span> officers
            </p>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
            ...
            {(search || activeFilters > 0) && (
              <button onClick={clearFilters} className="mt-3 text-xs text-blue-600 hover:underline font-semibold">
                Clear all filters
              </button>
            )}
          </div>

...
const INPUT = "w-full px-3.5 py-2.5 text-sm font-medium border border-slate-300 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors";
const LABEL = "block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wider";

function SectionHeading({ children }: { children: React.ReactNode }) {
    return (
        <div className="flex items-center gap-3 mt-2">
            <p className="text-xs font-black text-slate-500 uppercase tracking-[0.15em] whitespace-nowrap">{children}</p>
...
export default function CreateTrainIntelForm({ onClose, onSuccess }: Props) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] flex flex-col border border-slate-200">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 flex-shrink-0 bg-slate-50 rounded-t-2xl">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="font-black text-slate-900 text-sm">New Train Record</p>
                            <p className="text-xs font-medium text-slate-500">Train Crime Intelligence</p>
                        </div>
                    </div>
                ...
                <div className="overflow-y-auto px-6 py-5 space-y-5 flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className={LABEL}>Train Number <span className="text-red-600 ml-0.5 normal-case tracking-normal">*</span></label>
                            <input
                                type="text"
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    className={`${INPUT} appearance-none pr-9 font-bold ${RISK_COLORS[form.riskLevel]}`}
                                >
                                    {RISK_OPTIONS.map(r => (
                    ...
                    <div>
                        {form.crimeProfile.length === 0 && (
                            <p className="text-xs font-medium text-slate-500 mb-3 py-3 border border-dashed border-slate-300 rounded-lg text-center bg-slate-50">
                            ...
                            {form.crimeProfile.map((entry, i) => (
                                <div key={i} className="flex items-center gap-2">
                                    <input
                                        ...
                                        placeholder="Crime type (e.g. Child Rescue)"
                                    ...
                                    <input
                                        ...
                                        className="w-24 px-3.5 py-2.5 text-sm font-bold border border-slate-300 rounded-lg bg-white text-slate-900 text-center focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                        style={{ fontFamily: "'DM Mono', monospace" }}
                                        placeholder="0"
                                    />
                        ...
                        <button
                            ...
                            className="mt-3 flex items-center gap-1.5 text-xs font-bold text-blue-700 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 border border-blue-200 px-3 py-2 rounded-lg transition-colors"
                        >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {(Object.keys(RA_LABELS) as (keyof FormData["raCases"])[]).map(key => (
                            <div key={key}>
                                <label className="block text-xs font-semibold text-slate-600 mb-1.5">{RA_LABELS[key]}</label>
                                <input
                ...
                <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between flex-shrink-0 bg-slate-50 rounded-b-2xl">
                    ...
                    <div className="flex gap-2">
                        <button onClick={onClose} className="px-4 py-2.5 text-sm font-semibold text-slate-700 border border-slate-300 rounded-lg hover:bg-slate-100 transition-colors">
                            Cancel
                        </button>
                        <button

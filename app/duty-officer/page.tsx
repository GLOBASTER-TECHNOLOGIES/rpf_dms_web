...
type ModuleColor = "blue" | "teal" | "amber";
...
const modules: Module[] = [
  {
    href: "/duty-officer/view-train-details",
    icon: Train,
  ...
  {
    href: "/duty-officer/debriefing",
    icon: Mic,
    label: "Debriefing",
    badge: "Pending Report",
    color: "amber",
  },
  {
    href: "/duty-officer/view-circulars",
    icon: FileText,
    label: "HQ Circulars",
    badge: "2 Unread",
    color: "blue",
...
const badgeClass: Record<ModuleColor, string> = {
  blue: "bg-blue-100 text-blue-700",
  teal: "bg-teal-100 text-teal-700",
  amber: "bg-amber-100 text-amber-700",
};

const iconClass: Record<ModuleColor, string> = {
  blue: "bg-blue-50 text-blue-600",
  teal: "bg-teal-50 text-teal-600",
  amber: "bg-amber-50 text-amber-700",
};

const hoverBorder: Record<ModuleColor, string> = {
      ...
      <main className="flex-1 overflow-y-auto px-4 sm:px-6 py-5 pb-28 max-w-2xl w-full mx-auto flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-3">
          {modules.map(({ href, icon: Icon, label, badge, color }) => (
            ...
            >
              <div
                className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${iconClass[color]}`}
              >
              ...
              <span
                className={`text-xs font-semibold px-3 py-1 rounded-full flex-shrink-0 ${badgeClass[color]}`}
              >
                {badge}
              </span>
